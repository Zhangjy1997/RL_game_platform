...
from onpolicy.algorithms.NeuPL.train_PSRO import PSRO_learning
...
def parse_args(args, parser):
    ...
    parser.add_argument("--sub_round", type=int, default=2)
...
def main(args):
    ...
    if all_args.share_policy:
        from onpolicy.runner.shared.uav_psro_runner import UAV_Sigma_Runner as Runner
    else:
        from onpolicy.runner.separated.uav_psro_runner import UAV_Sigma_Runner as Runner
    ...
    if all_args.use_empty_policy:
        for i in range(all_args.population_size - 1):
            ...
            runner_e_list.append(copy.deepcopy(runner_e))
    else:
        runner_e = Runner(config)
        runner_e_list.append(copy.deepcopy(runner_e))
    ...
    with open('/home/qiyuan/workspace/flightmare_pe/flightrl/on-policy/onpolicy/scripts/train_uav_scripts/config_simple.json', 'r') as f:
        config_ssh = json.load(f)
    if all_args.use_wandb:
        PSRO_trainer = PSRO_learning(all_args, policy_anchor, share_policies,[policies_p1, policies_p2], [runner_p_list, runner_e_list], eval_match_envs,role_name,str(wandb.run.dir))
        if all_args.policy_backup_dir is None:
            ...
            #     PSRO_trainer.runners[0].inherit_policy(str(wandb.run.dir),'pursuer')
            # if seek_evader is False:
            #     PSRO_trainer.runners[1].inherit_policy(str(wandb.run.dir),'evader')
            if all_args.role_number == 0:
                if real_max_round == 0:
                    PSRO_trainer.warmup()
                for i in range(all_args.total_round):
                    ...
                    PSRO_trainer.run_single_round(begin_inx=real_max_round+1)
                    if PSRO_trainer.isTerminal():
                        break
                    print("Next round!")
                    real_max_round += 1
                ...
                for k in range(all_args.total_round):
                    ...
                    PSRO_trainer.eval_round_step(begin_inx=real_max_round+1)
                    ...
                    while check_and_download_files(config_ssh, remote_dir_next, str(wandb.run.dir),'sync.npy') is False:
                            for i in range(30):
                                ...
                                time.sleep(1)
                    PSRO_trainer.cal_graph(round_num=real_max_round+1, mat_sync= True)
                    if PSRO_trainer.isTerminal():
                        break
                    delete_files_with_suffix(str(wandb.run.dir),'sync.npy')
                    train_done = False
                    ...
                    while train_done == False:
                        PSRO_trainer.step_run(available_train_role=activate_role)
                        coeff_std = min(max(2, continue_inx), 6)
                        train_done = PSRO_trainer.check_training_effectiveness(coe_k=coeff_std, available_train_role=activate_role)
                        if all_args.role_number == 1:
                    ...
                    if all_args.role_number == 1:
                        ...
                        copy_pt_files(str(wandb.run.dir), round_dir,'pursuer.npy')
                        while check_and_download_files(config_ssh, remote_dir, round_dir,'evader.pt') is False:
                            for i in range(60):
                                print(f"Waiting... {i+1} seconds passed")
                                time.sleep(1)
                        while check_and_download_files(config_ssh, remote_dir, round_dir,'evader.npy') is False:
                            for i in range(60):
                                print(f"Waiting... {i+1} seconds passed")
                                time.sleep(1)
                    else:
                        copy_pt_files(str(wandb.run.dir), round_dir,'evader.pt')
                        copy_pt_files(str(wandb.run.dir), round_dir,'evader.npy')
                        while check_and_download_files(config_ssh, remote_dir, round_dir,'pursuer.pt') is False:
                            for i in range(60):
                                print(f"Waiting... {i+1} seconds passed")
                                time.sleep(1)
                        while check_and_download_files(config_ssh, remote_dir, round_dir,'pursuer.npy') is False:
                            for i in range(60):
                                print(f"Waiting... {i+1} seconds passed")
                                time.sleep(1)
