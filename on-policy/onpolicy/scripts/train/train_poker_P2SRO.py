...
from onpolicy.envs.poker.kuhn_poker_gym import kuhn_poker_symmetry as Env_K
...
from onpolicy.algorithms.NeuPL.train_P2SRO_symmetry import Neural_population_learning
...
def make_train_env(all_args):
    def get_env_fn(rank):
        def init_env():
            ...
            elif all_args.env_name == "KUHN_POKER":
                env = Poker(Env_K(all_args.n_rollout_threads))
            else:
...
def main(args):
    ...
    if all_args.share_policy:
        from onpolicy.runner.shared.poker_psro_runner import POKER_Runner as Runner
    else:
        from onpolicy.runner.separated.poker_psro_runner import POKER_Runner as Runner
    ...
    if all_args.use_wandb:
        NeuPL_trainer = Neural_population_learning(all_args, policy_anchor, share_policies, policies_p1, runner_p_list, evaluator, role_name, str(wandb.run.dir))
