...
import torch
import wandb
from onpolicy.algorithms.NeuPL.eval_match import eval_match_ball_final as eval_match
...
class Neural_population_learning:
    ...
    def __init__(self, args, anchor_policies, shared_policies, eval_policies, runners, evaluator, role_names, save_dir):
    ...
    def calu_effective_policy(self):
        ...
        max_num = min(self.frozen_inx +self.window_length , self.policy_num)
        ...
        max_num = min(self.frozen_inx +self.window_length -1, effect_p1_probs.shape[0], self.policy_num -1)
    ...
    def step_run(self):
        ...
        for i in range(len(self.effect_id_inx)):
            self.runners[i + self.frozen_inx].all_args.global_steps = self.g_step
            self.runners[i + self.frozen_inx].envs.world.oppo_policy = self.mix_policy[i]
            self.runners[i + self.frozen_inx].set_policy_inx(i + self.frozen_inx)
            self.runners[i + self.frozen_inx].run()
            self.g_step += self.num_env_steps
            self.runners[i + self.frozen_inx].save_as_filename("frozen_" + str(i + self.frozen_inx) + "_active")
    ...
    def check_training_effectiveness(self, coe_k = 3):
        ...
        for inx in range(len(self.effect_id_inx)):
            eval_values, standard_vaule = self.runners[self.effect_id_inx[inx]].get_payoff_sigma(self.n_eval_eps)
            print("eval_player1_policy {} = {}".format(self.effect_id_inx[inx], eval_values))
            ...
            if self.effect_id_inx[inx] == self.frozen_inx:
                ...
                self.runners[self.frozen_inx].save_as_filename("frozen_" + str(self.num_f) + "_backup")
    ...
    def eval_round_step(self, begin_inx):
        ...
        for j in range(1, eval_range):
            if j < self.frozen_inx:
                restore_eval_policy(self.eval_policies[j], self.save_dir, self.policies_shared[0].use_mixer ,head_str=("frozen_policy_" + str(j)))
            elif j == self.frozen_inx:
                max_inx = np.argmax(self.frozen_payoff_buffer)
                restore_eval_policy(self.eval_policies[j], self.save_dir, self.policies_shared[0].use_mixer ,head_str=("frozen_" + str(max_inx) + "_backup"))
            else:
                restore_eval_policy(self.eval_policies[j], self.save_dir, self.policies_shared[0].use_mixer, head_str=("frozen_" + str(j) + "_active"))
        ...
        if begin_inx <= 1:
            ...
            self.eval.get_win_prob_with_mask(self.n_threads, self.n_eval_eps, mask=self.mask_eval)
    ...
    def cal_graph(self, round_num):
        ...
        payoff_mat = payoff_mat - payoff_mat.T
        ...
        if self.terminal == False:
            self.mix_policy = []
            for inx in range(len(self.effect_id_inx)):
                self.mix_policy.append(mixing_policy(self.n_threads, self.eval_policies, probs= self.effect_ids[inx]))
