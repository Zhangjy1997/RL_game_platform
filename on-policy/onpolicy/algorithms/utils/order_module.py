...
class Order_Mixer(nn.Module):
    def __init__(self, args, role_name, device=torch.device("cpu")):
        ...
        self.num_others = self.num_team + self.num_oppo
        ...
        self.query_encoder=Encoder(self.hidden_size, self.n_pair * self.attn_size, self._layer_N, self._use_orthogonal, self._use_ReLU, self._use_feature_normalization)
        ...
        self.value_encoder=Encoder(2*self.hidden_size, self.attn_size, self._layer_N, self._use_orthogonal, self._use_ReLU, self._use_feature_normalization)
        self.key_encoder=Encoder(2*self.hidden_size, self.attn_size, self._layer_N, self._use_orthogonal, self._use_ReLU, self._use_feature_normalization)

        self.MH_attn=MultiHeadAttention(self.n_head, self.attn_size, self.d_k, self.d_k)
    ...
    def forward(self, obs):
        ...
        self.pair_code()
        self.query_code=self.query_encoder(obs_code['proprioceptive'])
        self.query_code=self.query_code.view(-1, self.n_pair, self.attn_size)
        obs_m, attn_m = self.MH_attn(self.query_code, self.key_codes, self.value_codes)
        obs_m=obs_m.view(-1, self.n_pair * self.attn_size)
        obs_out = torch.cat([obs_code['proprioceptive'], obs_m], dim = -1)
        return obs_out
    ...
    def output_space(self):
        return spaces.Box(low=-1.0, high=1.0, shape=((self.hidden_size+ self.n_pair * self.attn_size),), dtype=np.float32)
...
if __name__ == "__main__":
    ...
    for i in range(100):
        ...
        if torch.all(torch.abs(a-b) < 1e-5):
            flag_zero = True
        else:
            flag_zero = False
            print(a-b)
