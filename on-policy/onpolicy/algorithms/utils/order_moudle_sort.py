...
class Order_Mixer(nn.Module):
    def __init__(self, args, role_name, device=torch.device("cpu")):
        ...
        self.query_team_encoder=Encoder(self.proprio_shape, self.attn_size, self._layer_N, self._use_orthogonal, self._use_ReLU, self._use_feature_normalization)
        self.query_oppo_encoder=Encoder(self.proprio_shape, self.attn_size, self._layer_N, self._use_orthogonal, self._use_ReLU, self._use_feature_normalization)

        self.value_team_encoder=Encoder(self.teammate_shape, self.attn_size, self._layer_N, self._use_orthogonal, self._use_ReLU, self._use_feature_normalization)
        self.value_oppo_encoder=Encoder(self.opponent_shape, self.attn_size, self._layer_N, self._use_orthogonal, self._use_ReLU, self._use_feature_normalization)
    ...
    def forward(self, obs):
        ...
        if self.exist_team:
            self.query_team_code=self.query_team_encoder(self.obs_role_dict['proprioceptive'])
            self.query_team_code=self.query_team_code.unsqueeze(-2)

            obs_team, attn_team = self.MH_attn_team(self.query_team_code, self.value_team_codes, self.value_team_codes)
            attn_team=attn_team.squeeze(-2)
            attn_team = torch.mean(attn_team, dim=-2)
            # print(attn_team.shape)
            _, team_index = torch.sort(attn_team, dim = -1)
        ...
        if self.exist_oppo:
            self.query_oppo_code=self.query_oppo_encoder(self.obs_role_dict['proprioceptive'])
            self.query_oppo_code=self.query_oppo_code.unsqueeze(-2)

            obs_oppo, attn_oppo = self.MH_attn_oppo(self.query_oppo_code, self.value_oppo_codes, self.value_oppo_codes)
            attn_oppo=attn_oppo.squeeze(-2)
            attn_oppo = torch.mean(attn_oppo, dim=-2)
            # print(attn_oppo.shape)
            _, oppo_index = torch.sort(attn_oppo, dim = -1)
        ...
        # print("sort_index = ", self.sort_index)
        # obs_team=obs_team.squeeze(-2)
        # obs_out = torch.zeros_like(obs)
        # for i in range(obs.shape[0]):
        #     obs_out[i] = obs[i][self.sort_index[i]]
    ...
    def attn_code(self):
        ...
        for k in self.role_keys:
            if 'teammate' in k:
                team_code_list[team_num] = self.value_team_encoder(self.obs_role_dict[k])
                team_num += 1
            if 'opponent' in k:
                oppo_code_list[oppo_num] = self.value_oppo_encoder(self.obs_role_dict[k])
                oppo_num += 1
