...
from onpolicy.algorithms.r_mappo.algorithm.rMAPPOPolicy_share import R_MAPPOPolicy as Policy_share
from onpolicy.algorithms.r_mappo.r_mappo_share import R_MAPPO as TrainAlgo
import wandb
import imageio
...
class UAV_Share_Runner(Runner):
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            for step in range(self.episode_length):
                ...
                policy_head_str = ["policy_"+self.envs.world.team_name + str(self.porbs_inx[i])+"_" for i in range(self.n_rollout_threads)]
                policy_infos = []
                ...
                if self.all_args.use_mix_policy:
                    ...
                    if step< self.episode_length - 1:
                        ...
                        self.set_policy_inx(self.porbs_inx)
                    else:
                        # Delay training policy sampling until policy updates
                        done_ends = done_indices
            ...
            if episode % self.log_interval == 0:
                ...
                match_str_head = ["policy_"+self.envs.world.team_name + str(i)+"_" for i in self.id_inx]
                ...
                reward_logs = dict()
                num_logs = dict()
                # train_infos["policy_"+ self.envs.world.team_name + str(self.policy_num)+"_"+"average_episode_rewards"] = np.mean(self.buffer.rewards) * self.episode_length
    ...
    def set_policy_inx(self, indices):
        self.trainer.set_policy_inx(indices - 1)

    def set_id_sigma(self, id_sigma, id_inx = None):
        ...
        for i in range(self.n_rollout_threads):
            ...
            self.porbs_inx[i] = self.id_inx[random_inx]
        self.envs.world.oppo_policy.set_probs_mat(self.porbs_mat)
        self.set_policy_inx(self.porbs_inx)
    ...
    def checkVictory(self, info):
        ...
        evader_win = 0
        round_counter = 1
        pursuer_win_list = ["seize_evader_done", "out_of_range_done", "TimeLimit.truncated"]
        ...
        for k in info:
            ...
            if any(sub in k for sub in evader_win_list):
                ...
                # print(k)
                break
        evader_win += round_counter - (evader_win + pursuer_win)
        return pursuer_win, evader_win
    ...
    def calu_win_prob(self, total_episodes):
        ...
        for episodes in range(total_episodes):
            for eval_step in range(self.episode_length):
                ...
                for i in range(self.n_rollout_threads):
                    if eval_dones[i].all():
                        p_win, e_win = self.checkVictory(eval_infos[i])
    ...
    def get_payoff_sigma(self, total_episodes):
        ...
        for idx, row in zip(self.id_inx, self.id_sigma):
            self.set_policy_sigma(np.tile(row,(1,1)))
            self.envs.world.oppo_policy.set_probs_all(row)
            print("eval_{}_policy {}:".format(self.envs.world.team_name, idx))
            self.calu_win_prob(total_episodes)
            payoff_p = (self.total_pursuer_win - self.total_evader_win)/(total_episodes * self.n_rollout_threads)
            eval_payoffs[idx] = payoff_p
            prob_bar = self.total_pursuer_win / self.total_round
            Var_N = np.var(self.total_N_array, ddof=1) * total_episodes
            standard_vaules[idx] = np.sqrt(4*self.total_round*max(prob_bar*(1-prob_bar),0.05) + Var_N * (2*prob_bar -1) ** 2)/(total_episodes * self.n_rollout_threads)
