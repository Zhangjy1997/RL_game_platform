...
class UAV_Dual_Runner(Runner):
    ...
    def eval(self, total_num_steps):
        ...
        for eval_step in range(self.episode_length):
            ...
            if eval_dones[0].all():
                #exit(0)
                pursuer_win, evader_win, draw = self.checkVictory(eval_infos[0])
                if pursuer_win > evader_win:
                    print("pursuer win!")
                elif pursuer_win < evader_win:
                    print("evader win!")
                else:
                    print("draw!")
                break
            ...
            for eval_step in range(self.episode_length):
                ...
                for i in range(self.n_rollout_threads):
                    if eval_dones[i].all():
                        ...
                        self.total_round += 1
                        if self.all_args.use_mix_policy: 
                            self.eval_envs.world.oppo_policy.update_index_channel(i)
    ...
    @torch.no_grad()
    def eval_simple(self, total_num_steps):
        eval_episode_rewards = []
        self.oppo_record= []
        self.team_record= []
        eval_obs = self.eval_envs.reset()
        print("reset_obs = ", eval_obs)

        #self.position_record.append([eval_obs[0][0][0:3],eval_obs[0][1][0:3], eval_obs[0][2][0:3]])
        ob_op=copy.deepcopy(self.eval_envs.world.oppo_obs)
        self.oppo_record.append([ob_op[0][i][0:3] for i in range(self.eval_envs.world.num_oppo)])
        self.team_record.append([eval_obs[0][i][0:3] for i in range(self.eval_envs.world.num_team)])
