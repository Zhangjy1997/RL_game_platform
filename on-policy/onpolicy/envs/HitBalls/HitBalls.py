...
class PlankAndBall:
    def __init__(self, num_threads, cfg_path = None, oppo_policy=None):
        ...
        if cfg_path is None:
            cfg_path = '/home/qiyuan/workspace/flightmare_pe/flightrl/on-policy/onpolicy/envs/HitBalls/config.yaml'
    ...
    def update_nxt_state(self, action):
        plank1_vel = action[:, :2]
        plank1_heading = action[:, 2:3]
        plank2_vel = -action[:, 3:5]
        plank2_heading = action[:, 5:]

        plank1_vel = self.max_vel_plank * norm_batch(plank1_vel)
        plank1_heading = self.max_heading_plank * norm_batch(plank1_heading)
        plank2_vel = self.max_vel_plank * norm_batch(plank2_vel)
        plank2_heading = self.max_heading_plank * norm_batch(plank2_heading)
